...
public class StarbucksConsumer {
    ...
    public static void main(String[] args) {
        ...
        // Створюємо та запускаємо Consumer для власних потокових даних у окремому потоці
        Thread streamConsumerThread = new Thread(() -> {
            try (KafkaConsumer<String, StarbucksProduct> consumer = new KafkaConsumer<>(streamProps)) {
                consumer.subscribe(Arrays.asList(STREAM_TOPIC));
                ...
                while (true) {
                    ...
                    for (ConsumerRecord<String, StarbucksProduct> record : records) {
                        ...
                        if (product != null) {
                            logger.info("Потоковий запис: Ключ={}, Розділ={}, Зсув={}", 
        ...
        // Створюємо та запускаємо Consumer для даних з PostgreSQL у окремому потоці
        Thread dbConsumerThread = new Thread(() -> {
            try (KafkaConsumer<String, String> consumer = new KafkaConsumer<>(dbProps)) {
                consumer.subscribe(Arrays.asList(DB_TOPIC));
                ...
                while (true) {
                    ...
                    for (ConsumerRecord<String, String> record : records) {
                        ...
                        if (jsonStr != null) {
                            logger.info("БД запис: Ключ={}, Розділ={}, Зсув={}", 
                                    record.key(), record.partition(), record.offset());
